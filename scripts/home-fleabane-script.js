(function(){
    ...
    function reset(){
        ...
        for(let i = 0; i < branches.length; i++){
            while (branches[i].firstChild) {
                branches[i].removeChild(branches[i].firstChild);
            }
        }
        gen = 0;
        growGen();
    ...
    function genSeg(){
        ...
        const segs = getVal (2, 2.8);
        for(let i = 0; i < segs; i++){
            const newBranchNode = createDiv('node');
            if(segs == 2){
                newBranchNode.style.transform = `rotate(${(i * 2 - 1) * angle + Math.round(Math.random() * angleVar) - 0.5 * angleVar + 5}deg)`;
            }else{
                newBranchNode.style.transform = `rotate(${(i - 1) * angle + Math.round(Math.random() * angleVar) - 0.5 * angleVar + 5}deg)`;
            }
